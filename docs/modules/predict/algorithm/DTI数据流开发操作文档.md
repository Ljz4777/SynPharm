...
在开发DtiAlgoExecutor之前，以下文件必须已经存在：

| 序号 | 文件 | 状态 | 说明 |
|:---|:---|:---|:---|
| 1 | `enums/AlgoType.java` | ✅ | 算法类型枚举（已包含DTI） |
| 2 | `dto/ParsedInput.java` | ✅ | 统一输入格式DTO |
| 3 | `dto/request/PredictRequest.java` | ✅ | 预测请求DTO（含forDTI方法） |
...
│  阶段2: 实现算法执行器（组员完成）                                │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ ⑦ pipeline/impl/DtiAlgoExecutor.java → DTI执行器实现    │    │
│  └─────────────────────────────────────────────────────────┘    │
│                              ↓                                  │
│  阶段3: 自动注册（Spring完成）                                   │
│  ┌─────────────────────────────────────────────────────────┐    │
