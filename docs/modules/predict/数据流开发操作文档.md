...
@router.post("/predict", response_model=AlgoResponse)
async def predict_single(request: SingleRequest):
    if request.algo_type == "DTI":
        service = DTIService()
        metrics = service.predict(request.dict())
        return AlgoResponse(status="SUCCESS", metrics=metrics)
    else:
        # TODO: 添加PPI/DDI支持
        pass
...
**职责**：接收来自Spring Boot的预测请求，分发到对应的算法服务。
...
| 任务 | 文件路径 | 输入 | 输出 | 负责人 |
|:---|:---|:---|:---|:---|
| **PPIService** | `services/ppi_service.py` | protein_a, protein_b | PredictionMetrics | 算法组员A |
| **DDIService** | `services/ddi_service.py` | drug_a, drug_b | PredictionMetrics | 算法组员B |
...
| 任务 | 文件路径 | 说明 | 负责人 |
|:---|:---|:---|:---|
| **模型注册** | `core/loader.py` | 在ModelLoader中注册PPI/DDI模型 | 算法组员A/B |

**开发要求**：
- 在 `get_model()` 方法中添加PPI和DDI模型的加载逻辑
- 使用单例模式确保模型只加载一次
...
| 任务 | 文件路径 | 说明 | 负责人 |
|:---|:---|:---|:---|
| **路由添加** | `api/v1/predict.py` | 在预测路由中添加PPI/DDI的调用分支 | 算法组员A/B |

**开发要求**：
- 在 `predict_single()` 和 `predict_batch()` 中添加算法类型判断
- 调用对应的算法服务
...
| PPIService | FastAPI | [AI预测核心模块-FastAPI算法引擎技术开发文档.md](../AI预测核心模块-FastAPI算法引擎技术开发文档.md) |
| DDIService | FastAPI | [AI预测核心模块-FastAPI算法引擎技术开发文档.md](../AI预测核心模块-FastAPI算法引擎技术开发文档.md) |
