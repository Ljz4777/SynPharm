...
├── config/
│   └── RabbitConfig.java   # ✅已完成（RabbitMQ Exchange/Queue/死信配置）
├── mq/
│   ├── RabbitConfig.java       # ✅已完成（消息队列配置）
│   ├── BatchTaskMessage.java  # ✅已完成（消息体）
│   ├── BatchTaskProducer.java # ✅已完成（生产者）
│   └── BatchTaskConsumer.java # ✅已完成（消费者，手动 ack）
├── exception/
...
#### 2.1.3 RabbitMQ 消息队列异步实现

```java
// 文件: mq/RabbitConfig.java
@Configuration
@EnableRabbit
...
**生产者**（`mq/BatchTaskProducer.java`）：`uploadBatch` 落库成功后投递消息。

**消费者**（`mq/BatchTaskConsumer.java`）：`@RabbitListener(ackMode="MANUAL")`，处理成功 `basicAck`，失败 `basicNack(requeue=false)` 进入死信队列。

**使用方式**：由消息队列驱动异步执行（替代原 `@Async` 线程池方案），任务状态以 `batch_task` 表为权威。

#### 2.1.4 进度存储（DB 为权威，Redis 仅缓存）

> ⚠️ **v3.1.0 变更**：进度/状态以 `batch_task` 表为权威（含 `algo_type`/`deleted` 列），Redis 仅作进度回显缓存，重启/过期不影响正确性。下方为旧实现，仅供理解演进。
...
**现阶段重点**：确保路由能正确匹配三种算法类型，具体实现（模型推理、特征提取）可后续补充。

#### 3.2.1 路由匹配（✅已完成）

当前路由已完整支持三种算法类型的匹配：

**单条预测路由** (`api/v1/predict.py`)：
...
**算法选择器** (`services/batch_service.py`)：
...
| 请求 | 路由 | 算法类型 | 匹配结果 |
|:---|:---|:---|:---|
| `{"algo_type": "DTI", "drug_smiles": "...", "target_seq": "..."}` | `/v1/predict/single` | DTI | ✅匹配成功 |
| `{"algo_type": "PPI", "protein_a": "...", "protein_b": "..."}` | `/v1/predict/single` | PPI | ✅匹配成功 |
| `{"algo_type": "DDI", "drug_a": "...", "drug_b": "..."}` | `/v1/predict/single` | DDI | ✅匹配成功 |
| `{"algo_type": "XXX", ...}` | `/v1/predict/single` | 未知 | ❌返回错误 |

#### 3.2.2 算法服务实现（后续完善）

三个算法服务的基础框架已完成，包含mock数据返回，待后续完善真实模型推理逻辑：

| 服务 | 文件路径 | 状态 | 负责人 |
|:---|:---|:---|:---|
| DTIService | `services/dti_service.py` | ✅路由匹配已完成 | 算法组员A |
| PPIService | `services/ppi_service.py` | ✅路由匹配已完成 | 算法组员B |
| DDIService | `services/ddi_service.py` | ✅路由匹配已完成 | 算法组员C |
