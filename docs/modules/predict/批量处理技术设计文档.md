...
前端 `api` 层当前**未调用批量接口**，需新增（后续实现）：

```ts
// api/predict.ts 或独立 batch.ts
export const batchApi = {
  upload(file: File, algoType: string): Promise<{ batchId: string; totalCount: number; status: string }> {
...
| 消费者 | `batch-predict-` 线程名 + 日志追踪 batchId |
...
| 步骤 | 内容 | 依赖 |
|---|---|---|
| 1 | docker-compose 加 rabbitmq + 网络/数据卷 | - |
| 2 | pom 加 `spring-boot-starter-amqp` + application 配置 | 1 |
| 3 | `batch_task` 加列（`algo_type`/`deleted`）+ 迁移 SQL + 实体 | - |
| 4 | 新增 `mq/` 包（RabbitConfig/Producer/Consumer） | 2 |
| 5 | `BatchProcessServiceImpl` 改造：upload 发消息、processBatch 幂等、状态落库 | 3,4 |
| 6 | status/download 归属校验 + 恢复扫描（启动时重新投递 PENDING/PROCESSING） | 5 |
| 7 | 前端 batch API + 批量上传交互 | 5 |
| 8 | 测试（单测/集成/E2E）+ 文档同步 | 全部 |
...
| `service/impl/AsyncConfig.java` | 可移除（不再使用 @Async 批量） |
