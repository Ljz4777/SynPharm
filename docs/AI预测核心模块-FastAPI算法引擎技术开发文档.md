...
FastAPI算法引擎是 SynPharm AI预测核心模块的**无状态计算节点**，负责加载论文中的推理文件（PyTorch模型），执行GPU推理，提供 `/v1/predict/single` 和 `/v1/predict/batch` 接口供SpringBoot业务中台调用。
