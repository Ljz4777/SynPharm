...
| 会话表 / 步骤表 | 新建 DDL | `sql/NN_*.sql` 编号延续（从 `10_` 起） |
...
延续现有 DDL 编号规范（当前最大为 `09_*`），新增 4 张表。
...
### 13.6 批量 CSV 字段错位（低，与 Agent 无直接关系）
...
sql/{10_agent_session,11_agent_step,12_agent_report,13_tool_invocation_log}.sql
```
