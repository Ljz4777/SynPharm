...
> **注意**：上表中**除 `上线流程详细版.md` 外**的来源文档均已归并完毕，并已**从仓库删除**
> （只保留在 git 历史中）—— 避免同一问题在两处维护、互相漂移。
> 2026-09-23 再次清理了 3 份已归并文档：`用户认证模块代码审查日志_20260722.md`、
> `个人中心待建功能模块技术方案.md`、`组员任务清单.md`。
> 本表保留为**归并溯源记录**，需要时可按此回查 git 历史。
...
### 3.0 2026-09-22 本轮已修复（v3.3.1）

| ID | 内容 |
|---|---|
| `E-01` | `/health` 恒返回 healthy → 改为逐算法报告就绪状态（`ready`/`not_loaded`/`weights_missing`/`load_failed`），刻意不触发加载，HTTP 码仍 200 |
| `E-02` | 模型加载失败永久缓存 `None`（补权重必须重启） → 冷却 300 秒后可重试 |
| `E-03` | 404 被吞成 500 → 已分类的 `PredictionError` 原样透出 |
| `E-06` | `async` 端点里做阻塞推理 → `/single`、`/batch`、`/ddi/drugs` 改同步 `def`，交给线程池 |
| `D-04` | 前端整体丢弃非 2xx 响应体 → 透出后端 `message` + `errorCode` + HTTP 状态码 |
| `A-04` | 前端仍调已废弃的 `/dti`\|`/ppi`\|`/ddi` → 改走 `/api/predict/general` |
| `G-01` | "保存结果"按钮无 `@click` → 接入 `POST /api/favorites` |
| `G-02` | "高级选项"两控件不生效 → 阈值提示 + 详细结果开关真正生效（标注为前端判定） |
| `G-06` | 结果面板串到批量/历史页签 → `v-if` 补 `mode === 'single'` |
| `G-10` | 批量轮询失败静默中止 → 给出明确错误提示 |
| `C-05` | `PredictUtils` 用 `Random` 造假 → **已缓解未清除**：`predict()` 改为抛错，假数据不可达；三个私有造假方法与 `executeTask` 仍是死代码，待专门清理 |

> 完整的修复记录、验证证据与仍未修复清单见 `log/CHANGELOG.md`（v3.3.1）与
> `docs/development/未修复功能清单.md`。
