...
修复前端镜像**不构建分子编辑器子应用**的缺陷，并把本轮设计工作台的成果重新部署到 Docker。

### 缺陷：前端镜像缺少化学引擎子应用（构建不可复现）

**现象**：Docker 里跑的前端没有本轮新功能（导航栏仍是旧顺序）；即使重建镜像，编辑器也会打不开。

**根因（两层）**：

1. **镜像陈旧** —— `scripts/start.bat` 设计上「镜像已存在则跳过构建」
   （避免构建缓存被回收时重下 185 MB 的 torch），所以改完源码必须显式 `scripts\start.bat rebuild`。
   本次前端镜像停留在 2 天前，自然没有设计工作台的提交。
2. **更严重：Dockerfile 从不构建子应用** —— `RUN npx vite build` 只构建主应用，
   而分子编辑器子应用（Ketcher + Indigo，约 29 MB）的产物 `public/ketcher/` 在 `.gitignore` 内。
   此前之所以"能跑"，纯粹是因为开发者本地恰好存在一份旧产物被 `COPY . .` 带了进去 ——
   **一旦在 CI 或新克隆的机器上构建，编辑器必然白屏**。

**修复**：

- `Dockerfile` 在 `vite build` **之前**新增 `RUN npm run build:ketcher`，子应用改为**镜像内构建**
- `.dockerignore` 排除 `public/ketcher/`，防止本地陈旧产物静默混入镜像

### 部署结果（Docker 实测）

| 项 | 结果 |
|---|---|
| 镜像内构建 | 两次 Vite 构建均执行：`#13` → `public/ketcher/`（含 29,093 kB 引擎分块）、`#14` → `dist/` |
| 导航栏 | 7 项，**设计工作台居首**，无「3D可视化」 |
| 分子编辑器 | `编辑器就绪`；iframe `/ketcher/index.html` 返回 200 |
| 引擎实算 | **本页 18/18 已引擎校验**（证明 29 MB 引擎分块正确构建并可加载） |
| 3D 口袋页签 | 3W2S 结构加载，口袋腔体与共结晶配体开关正常 |
| nginx gzip | 引擎分块 **29.1 MB → 8.3 MB**（`Content-Encoding: gzip`） |
| 控制台 | 无 error |

> **操作提示**：改完前端源码后必须 `scripts\start.bat rebuild`，否则新功能不会上线
> （`deploy/环境配置说明.md` 与 `README.md` 均已有说明）。

## [v3.4.1] - 2026-09-24

### 今日主题

把 3D 可视化的**显示控制接口收口到一处**，并按"设计工作台即第一阶段交付物"调整导航栏。

### 3D 控制接口对齐（重构）

`Visualization.vue`（预测域 3D 页）与 `PocketView.vue`（工作台口袋页签）此前**各写一份**
同一套接口，改一处必漏另一处。现抽出 `components/protein/viewerControls.ts` 作为唯一来源：

| 收口到共享模块 | 说明 |
|---|---|
| `MOLSTAR_REP_TYPES` / `MOLSTAR_COLOR_TYPES` | 显示模式 / 颜色方案 → Mol* 内部类型映射 |
| `VIEWER_DISPLAY_MODES` / `VIEWER_COLOR_SCHEMES` | UI 直接渲染的数据（含预览渐变） |
| `MolstarHandle` | 组件句柄接口（两处原各写一份） |
| `useMolstarControls()` | 切换模式/配色 + **结构加载后重新同步**的公共逻辑 |

- 两个页面的**设置项列表刻意不统一**：口袋页签多出「口袋腔体」「共结晶配体」，
  那是真实的界面差异；本次只统一映射与同步逻辑
- `Visualization.vue` 的 ref 类型由 `InstanceType<typeof MolstarViewer>` 改为显式
  `MolstarHandle`，与工作台（用的是异步组件、拿不到组件类型）保持一致
- 纯重构，行为不变

### 导航栏调整

- **设计工作台（`/design`）移到导航栏第一位** —— 它是第一阶段的主要交付物
- **移除「3D 可视化」导航项**（`/visualization`）：其 3D 展示能力已并入工作台口袋页签
- ⚠️ **路由与页面保留**：该页被 `Dashboard` / `Predict`（2 处）/ `ResultDetail` / `Results`
  共 **5 处**深链引用，且会带上预测上下文（亲和力、置信度、相互作用详情）——
  那属于**预测域**数据，而工作台口袋页签用的是**设计域**数据，替代不了
- 导航项由 8 项减为 7 项，恰好对上窄屏断点注释里"放不下 7 项"的描述

### 文档

- `README.md`：3D 可视化标注为"由预测结果页深链进入"；页面清单标注导航可见范围
- 新增 **`docs/SynPharm项目书.md`**（竞赛 / 申报用）：含背景与痛点、竞品对比、核心创新点、
  系统架构、技术方案、**诚实的完成度说明**、七项关键技术难点、可行性分析、路线图与风险应对
...
|---|---|
| 导航栏 | 7 项，设计工作台居首；无「3D可视化」 |
| 显示模式切换 | 点击「球体」→ `[Molstar] cartoon -> spacefill` |
| **结构切换后重新同步** | 3W2S → 1M17 加载后自动重放为 `spacefill` 与 `element-symbol`（界面状态与画面一致） |
| 页内独有项 | 「显示网格」等口袋页签专属设置仍正常 |
| 旧路由可达 | 直接访问 `/visualization` 正常渲染，含显示控制面板 |
| 前端类型检查 | `vue-tsc --noEmit` 通过 |
| 生产构建 | `vite build` 退出码 0 |
| 控制台 | 无 error |
...
交付**干实验设计工作台**（第一阶段形态：类 IDE 的手工工具台），并把它与既有 3D 可视化能力打通。
后端接口尚未落地，当前由**可切换的内置演示数据**驱动。

### 设计工作台（新增，10 个提交）

1. **页面骨架**（`7702b7e`）—— `/design` 三栏 IDE（左：项目/靶点/约束/轮次；中：四页签；右：约束判定 + 指标）。
   **纯增量接入**：只改 `router/index.ts`（+1 路由）与 `Sidebar.vue`（+1 导航项），
   既有 7 个页面、`App.vue`、全局样式一律未动。

2. **2D 分子编辑器**（`5515fb0`）—— Ketcher 走**独立子应用 + iframe**（方案 B）。
   约 29 MB 的 Indigo 化学引擎完全隔离在子文档内，主应用构建配置零改动。
   踩中并解决三个坑：Node 内置模块缺失、ESM 产物里的 `require('raphael')`、
   大分子编辑器（同样依赖 Raphael）的干扰。

3. **数据层**（`df2d4c7`）—— `types/design.ts` 契约 + `api/design.mock.ts` **生成式**演示数据 +
   `api/design.ts` 门面 + `stores/design.ts`。页面**零写死数据**，
   后端就绪后只置 `VITE_DESIGN_MOCK=false` 即可切换。
   约束判定与综合得分是**真实逻辑**：11 条规则逐条判、按评估器文档的加权公式算分、
   硬规则不过则按「得分不算」置 0。

4. **对比页签**（`5c4b3e2`）—— 分子级并排结构式 + 指标差值；轮次级 ECharts 趋势 + 统计 + 谱系流向。

5. **口袋页签**（`cf8e898` → `646ef61` → `5dfa983`）—— 从 RCSB 加载真实结构，补齐全套 3D 显示控制，
   再新增**口袋腔体线框球**（`binding_pocket.center / radius` 的可视化）与**共结晶配体**球棍渲染。
   后两项对 `MolstarViewer.vue` 是**可选 prop 的纯增量扩展**，不传即完全不生效。

6. **候选分子引擎实算**（`8fb3974`）—— 编辑器子应用新增 `analyzeBatch` 协议，
   用本地 Indigo 实算 **InChIKey / 分子量 / 分子式** 并回填数据层
   （InChIKey 因此成为可用的平台级去重键）。编辑器就绪即自动校验当前页（实测 2 秒内 18/18），
   引擎未就绪时自动降级回估算值。
   顺带修正一处数据保真度缺陷：脚手架把连接点写在**开头**再做替换，导致取代基**反向接入**
   （标签 `COOH` 实为甲酸酯、`CH2OH` 实为甲氧基），已改为尾部拼接。

### 部署 / 配置

- **nginx 开启 gzip**（`6052294`）：编辑器子应用入口分块 29 MB → 约 8.5 MB
- **修正开发态后端地址**（`47841b6`）：`.env` 的 `VITE_API_BASE_URL` 由 `8080` 改为与实际一致的 `7000`，
  并补齐 `.env.example` 说明（该端口必须与 `deploy/.env` 的 `BACKEND_PORT` 对齐）

### 文档

- 新增 `docs/modules/design/` **8 份**模块设计文档（总览与架构分工、项目管理、靶点与口袋、
  设计约束、分子生成器、分子编辑器、评估器、工作台与交互设计）
- 明确**第一阶段交付物为干实验 IDE**；架构铁律：SpringBoot 负责业务与持久化，
  **一切计算归 FastAPI**（且 FastAPI 不访问数据库）
- 更新 `README.md`：补充设计工作台、前端技术栈（Ketcher / Molstar / ECharts）、目录结构、
  启动步骤（含 `build:ketcher`）、设计接口契约与文档索引；
  修正 TypeScript 版本（5.6 → 5.3）与前端开发端口（5173 → 5174）

### 验证

| 项 | 结果 |
|---|---|
| 2D 编辑器 | 子应用就绪；可读写分子并渲染结构式（`generateImage` 已串行化） |
| 引擎实算 | 甲苯 `YXFVVABEGXRONW-UHFFFAOYSA-N`、苯甲醇 `WVDDGKGOMKODPV-UHFFFAOYSA-N`（与公开值一致） |
| 自动校验 | 编辑器就绪后 2 秒内本页 18/18 完成实算 |
| 演示数据 | R1 = 132 个候选（通过率 14.4%）；R2 = 18 个（通过率 50%），父子谱系真实 |
| 3D 视图 | 3W2S / 1M17 真实结构加载；腔体线框球与配体球棍开关均生效 |
| 前端构建 | `vue-tsc --noEmit` 通过；`vite build` 退出码 0 |
| 回归 | 既有页面未受影响（本轮仅新增路由与导航项） |

## [v3.3.1] - 2026-09-22

### 今日主题

修复算法引擎健壮性与前端功能缺口，并补齐"未修复功能清单"文档。

### 引擎健壮性

1. **`/health` 恒返回 healthy（E-01）**
   - 此前与真实就绪情况完全无关：权重全缺失时也报 healthy，
     上游熔断器据此判定引擎正常，排障时只能看到"healthy 但所有预测都 404"
   - 现在返回逐算法状态（`ready` / `not_loaded` / `weights_missing` / `load_failed`）与 summary
   - 刻意**不触发加载**（PPI 权重 2.8GB，探活不能是重活）；HTTP 状态码仍保持 200，
     避免因某个算法缺权重就把容器健康状态打掉

2. **模型加载失败永久缓存 `None`（E-02）**
   - 原先失败会永久缓存，补上权重后**必须重启容器**才能恢复
   - 改为冷却 300 秒后允许重试：既避免每个请求都触发昂贵加载，又能自愈

3. **`async` 端点里做阻塞推理（E-06）**
   - 推理是 CPU 密集的阻塞调用，写在 async 里会卡住事件循环，连 `/health` 都响应不了，
     会被上游熔断器当成引擎故障
   - `/v1/predict/single`、`/v1/predict/batch`、`/v1/ddi/drugs` 改为同步 `def`，交给线程池

### 前端功能缺口

4. **整体丢弃非 2xx 响应体（D-04）**
   - 此前调用方只能拿到 `Request failed with status code 400`，
     **后端精心写的中文原因 100% 丢失**
   - 现在透出后端 `message`，并把 `errorCode` 与 HTTP 状态码挂到 Error 上；
     无响应体时给出中文兜底

5. **仍在调用已废弃接口（A-04）**
   - `/api/predict/dti|ppi|ddi` 已标注 `@Deprecated`，前端改走 `/api/predict/general`
   - 顺带接上新增的 `GET /api/predict/ddi/drugs`（`predictApi.getDdiDrugs`）

6. **"保存结果"按钮无任何 `@click`（G-01）**
   - 接入 `POST /api/favorites`，带保存中 / 已保存态；结果无 ID 时禁用并给出 title 提示

7. **"高级选项"两个控件从不生效（G-02）**
   - 置信度阈值：低于阈值时在结果区显示"低于阈值 X%"徽标
   - 输出详细结果：关闭后隐藏"相互作用分析"区块
   - 两者均为**前端判定**，已在文案中如实标注，不假装引擎使用了这些参数

8. **结果面板串到批量/历史页签（G-06）**：`v-if` 补上 `mode === 'single'`

9. **批量轮询失败静默中止（G-10）**
   - 此前是空的 `catch`，轮询一失败就停表，进度条永久停在原地、用户完全不知道发生了什么
   - 现在给出明确错误提示

### 假数据

10. **`PredictUtils` 造假数据（C-05，已缓解未清除）**
    - 该类用 `Random` 编造靶点、亲和力、置信度和相互作用，被 `TaskServiceImpl#executeTask` 调用
    - 已确认 `executeTask` **无任何 Controller 调用**，属不可达死代码
    - 本次让它直接抛 `UnsupportedOperationException`，任何误用立刻暴露，
      不再静默产出"看起来完全正常"的随机报告
    - ⚠️ 三个私有造假方法仍是死代码，待专门清理提交连同 `executeTask` 一起删除

### 文档

- 新增 `docs/development/未修复功能清单.md`：一页看清剩余约 102 条问题，
  区分"能立刻做（约 96 条）"与"必须等外部数据（6 条）"
- 新增 `docs/development/测试数据准备清单.md`（上一提交）

### 验证

| 项 | 结果 |
|---|---|
| `/health/` | 返回逐算法状态；三算法 `weights_present: true` |
| 三算法加载 | DTI / DDI / PPI 全部 `ready`；DDI `0.662937`；白名单 `1323` |
| DDI 走统一入口 | `200`，`0.6629`，`targetId=DB00880-DB09220` |
| 批量链路回归 | 上传 2 行 → `SUCCESS`、`successCount=2`、`failCount=0`、`progress=100` |
| 结果 CSV | 表头 + 2 行真实数据，首行 `DB00880,DB09220,0.6629,medium` |
| 前端 | `200`，返回真实构建产物（`index-BZ9xx_1P.js`） |
| 前端类型检查 | `vue-tsc --noEmit` 通过 |
| 后端编译 | `mvn package` 通过 |
